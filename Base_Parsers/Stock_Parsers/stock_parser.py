...
def stock_parser_worker(ticker, curr_idx, df_index_ptr, start_date=None, end_date=None):
    ...
    base_data_frame = pd.DataFrame(
        columns=['tickerId', 'date', 'open', 'high', 'low', 'close', 'adjclose', 'volume'])
    ...
    try:
        ...
        current_data_frame = stock_info.get_data(
            ticker[2], index_as_date=False, start_date=start_date, end_date=end_date)
    ...
    if not e_flag:
        ...
        base_data_frame = pd.concat(
            [base_data_frame, current_data_frame], axis=0)

    # Remove any rows with missing data and reset the index
    base_data_frame.dropna(inplace=True)
    base_data_frame.index = range(
        df_index_ptr, df_index_ptr + base_data_frame.shape[0])
...
def stock_parser(ticker, start_date=None, end_date=None):
    ...
    base_list, curr_idx, df_index_ptr = stock_parser_worker(
        ticker, curr_idx, df_index_ptr, start_date=start_date, end_date=end_date)
